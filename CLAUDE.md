...
3. Claudeがチェック済みの全テーマについて、角度ブレストをスキップしてthemes.mdの切り口案をそのまま使い、Step 2〜3（具体↔抽象の設計 → 執筆）を順番に実行してdraftを生成する
...
- **英語記事（海外投資家向け）**: 月1本ペースで仕込む。POS SaaSの翻訳ではなく「投資商品としての日本ナイトビジネス」が主軸。キャバクラに限定せずナイトクラブ・バー・ホストクラブ等を含む広い切り口で書く。不動産投資との比較、M&A・事業売買、風営法・ビザ要件など。競合コンテンツがほぼゼロなので先行者ポジションを取れる。Luna POSへの言及は自然な文脈で軽く（「運営にはこういうツールがある」程度）
