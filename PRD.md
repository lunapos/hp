...
- **ツールチップ・インラインヘルプ**: 複雑な入力項目（税率・サービス料率・指名料設定等）にはフィールド横に `?` アイコン + ツールチップで説明を表示
- **明確なエラーメッセージ**: 「エラーが発生しました」ではなく「サービス料率は0〜100の数値を入力してください」のように具体的な修正方法を提示
- **空状態ガイド**: データ0件時に「まずはキャストを登録しましょう」等の次のアクションを案内
- **オンボーディング**: 初回ログイン時にステップバイステップのセットアップガイド（店舗設定→テーブル→キャスト→メニュー→セットプランの順）
